...
class HW2_sql():
    ...
    def part_ai_1(self,connection):
        ...
        part_ai_1_sql = "create table if not exists movies( id integer PRIMARY KEY,title text, core real);"
        ######################################################################
    ...
    def part_aii_1(self,connection,path):
        ...
        with open('./data/movies.csv', newline='') as csvfile:
            ...
            for row in spamreader:
                id, title, core = row
                connection.execute("INSERT INTO  movies VALUES (?,?,?)",(int(id),title,core))
                connection.commit()
       ######################################################################
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_sql = "create table if not exists  cast_bio(cast_id integer ,movie_cast text, cast_name text,birthday text,popularity);"
        ######################################################################
        
        self.execute_query(connection, part_aiii_sql)
        
        ############### CREATE IMPORT CODE BELOW ############################
        part_aiii_insert_sql = "INSERT INTO cast_bio(cast_id,cast_name,birthday,popularity)SELECT er.cast_id,er.cast_name,er.birthday, er.popularity FROM movie_cast er GROUP BY cast_id;"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "WITH total_actors AS (SELECT COUNT(*) as count FROM movie_cast WHERE birthday != 'None'),actors_born_in_range AS (SELECT COUNT(*) as count FROM movie_cast WHERE birthday != 'None' AND birthday BETWEEN '1965-01-01' AND '1985-12-31') SELECT (actors_born_in_range.count * 100.0) / total_actors.count AS proportion FROM total_actors, actors_born_in_range"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name,COUNT(movie_id)FROM movie_cast WHERE popularity > 10 GROUP BY cast_name ORDER BY COUNT(movie_id) DESC,cast_name LIMIT 5"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT title,printf('%.2f',score) AS movie_score,COUNT(cast_id) as cast_count from movies INNER JOIN movie_cast on movie_cast.movie_id=movies.id GROUP By movies.id ORDER BY score DESC, cast_count ASC,title ASC LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "select cast_id,cast_name As cast_name,printf('%.2f',avg(score)) FROM movie_cast INNER JOIN movies on movie_cast.movie_id=movies.id where movies.score>25 GROUP by cast_id HAVING COUNT(movie_id)>2 ORDER BY avg(score) DESC,cast_name ASC LIMIT 10"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        with open('./data/movie_overview.csv',errors = 'ignore') as csvfile:
            ...
            for row in file:
                connection.execute("INSERT INTO movie_overview values (?,?)",
                (row))
                connection.commit()
...
if __name__ == "__main__":
    ...
    db = HW2_sql()
    try:
        conn = db.create_connection("Q2.sqlite")
    except:
