...
#
# The `TMDbAPIUtils` class is used to retrieve Actor/Movie data using themoviedb.org API.  We have provided a few
# necessary methods to test your code w/ the API, e.g.: get_movie_cast(), get_movie_credits_for_person().  You may
# add additional methods and instance variables as desired (see Helper Functions).
#
...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> List[Tuple[Any, Any]]:
    ...
    def add_edge(self, source: str, target: str) -> List[Tuple[Any, Any]]:
        """
...
def get_api_data(endpoint: str) -> list:
    ...
    api_version = '/3/'
    connection = http.client.HTTPSConnection(f'{host}', 443)
    connection.request("GET", f'{api_version}{endpoint}')
    response = connection.getresponse()
    data = response.read()
    new_data = json.loads(data)
    connection.close()
    return new_data["cast"]
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list = None) -> list:
        ...
        """
        endpoint = f"movie/{movie_id}/credits?api_key={self.api_key}&language=en-US"
        casts = get_api_data(endpoint)
        # return movie credits with given range
        if limit:
            return [
                {
                    data: value
                    for data, value in casts[iteration].items()
                    if casts[iteration]["order"] < limit and data in ("id", "character", "credit_id")
                }
                for iteration in range(len(casts))
            ]
        # exclude any id in the movie credits
        if exclude_ids:
            return [
                {
                    data: value
                    for data, value in casts[1].items()
                    if casts[iteration]["id"] in exclude_ids and data in ("id", "character", "credit_id")
                }
                for iteration in range(len(casts))
            ]
        # return all data incase no **kwargs given
        return [
            {
                data: value
                for data, value in casts[iteration].items()
                if data in ("id", "character", "credit_id")
            }
            for iteration in range(len(casts))
        ]
    ...
    def get_movie_credits_for_person(self, person_id: str, vote_avg_threshold: float = None) -> list:
        ...
        endpoint = f"/person/{person_id}/movie_credits?api_key={self.api_key}&language=en-US"
        casts = get_api_data(endpoint)
        ...
        if vote_avg_threshold:
            return [{
                data: value
                for data, value in casts[iteration].items()
                if casts[iteration]["vote_average"] >= vote_avg_threshold and data in ("id", "title", "vote_average")
        ...
        return [
            {
                data: value
                for data, value in casts[iteration].items()
                if data in ("id", "title", "vote_average")
            }
            for iteration in range(len(casts))
        ]
...
if __name__ == "__main__":
    # remove multiple empty object in a list
    def remove_empty_object(data: list) -> list:
        return [dict(contents) for contents in {tuple(ob.items()) for ob in data if ob}]


    # BEGIN BUILD CO-ACTOR NETWORK
    graph = Graph()
    graph.add_node(id='2975', name='Laurence Fishburne')
    key = "46db37ad845d98c4f378a9ea07bcbd9c"
    tmdb_api_utils = TMDBAPIUtils(api_key=f'{key}')
    ...
    # END BUILD BASE GRAPH
    #   Find all of Laurence Fishburne's movie credits that have a vote average >= 8.0
    credit_above_8 = tmdb_api_utils.get_movie_credits_for_person('2975', 8.0)
    # remove empty {} from the list
    first_persons = remove_empty_object(credit_above_8)
    ...
    for item in first_persons:
        try:
            cast_members_Order_below3 = tmdb_api_utils.get_movie_cast(item['id'], 3)
            for member in remove_empty_object(cast_members_Order_below3):
                graph.add_node(id=str(member['id']), name=member['character'])
                # Keeping track of all new nodes added
                initial_nodes.append((member['id'], member["character"]))
                graph.add_edge(source='2975', target=str(member['id']))
            continue
    ...
    def generate_nodes(nodes: list):
        for each in nodes:
            source_edge = each[0]
            Movie_credit_above_8 = remove_empty_object(
                tmdb_api_utils.get_movie_credits_for_person(each[0], 8.0)
            )
            for credit in Movie_credit_above_8:
                try:

                    cast_members_Order_below_3 = remove_empty_object(
                        tmdb_api_utils.get_movie_cast(credit['id'], 3))
                    for cast_member in cast_members_Order_below_3:
                        person = (str(cast_member["id"]), cast_member['character'])
                        graph.add_node(*person)
                        new_nodes.append(person)
                        graph.add_edge(str(source_edge), str(person[0]))
                        continue
                except Exception as e:
                    print(e)
